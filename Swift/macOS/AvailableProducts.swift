...
Abstract:
A PrimaryViewController subclass that displays a list of products available for sale in the App Store. Displays the localized title and
 price of each of these products using SKProduct. Indicates whether each product is hosted or not. Calls StoreObserver to implement a purchase when a
 user taps a product.
*/
...
class AvailableProducts: PrimaryViewController {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        case .hosted: cell.textField?.stringValue = (product.isDownloadable) ? Hosted.yes.description : Hosted.not.description
        case .price:
